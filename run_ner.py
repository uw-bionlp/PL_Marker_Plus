...
class ACEDatasetNER(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False):
        ...
        elif args.data_dir.find('recommendation')!=-1:
            self.ner_label_list = ['NIL','Recommendation', 'Rec_Type', 'Rec_TimeFrame', 'Rec_Anatomy', 'Rec_Reason', 'Rec_Test']
        elif args.data_dir.find('incidentaloma')!=-1:
            self.ner_label_list = ['NIL','Medical_Problem', 'Lesion', 'Indication', 'Size', 'Characteristic', 'Count', 'Anatomy', 'Size_Trend']
                # 'Indication_Type', 'Assertion', 'Size_Trend', ]
        elif args.data_dir.find('assertion')!=-1:
            self.ner_label_list = ['NIL','present', 'absent', 'possible']
        elif args.data_dir.find('anatomy_parent')!=-1:
            self.ner_label_list = ['NIL','Head_Neck', 'Cardiovascular', 'Abdomen', 'M_Reproductive', 'Urinary', 'Body_Regions', 'Respiratory', 'Miscellaneous',
            'Skin', 'F_Reproductive_Obstetric', 'Neurological', 'Digestive', 'Lymphatic', 'Musculo-Skeletal', 'Thoracic', 'Hepato-Biliary']
        elif args.data_dir.find('anatomy_child')!=-1:
            self.ner_label_list =['NIL','Connective_Tissue', 'Laryngeal', 'Spine_Thoracic', 'Cerebrovascular_System', 'Skin_and_or_Mucous_Membrane', 'Pharynx', 'Spleen', 'Gallblader', 'Intestine', 'Extraaxial', 'Ureter', 'Nasal_Sinus', 'Eye', 'Pleural_Membrane', 'Subcutaneous', 'Entire_Body', 'Abdominal_Wall', 'Adnexal', 'Pituitary', 'Small_Intestine', 'Ear', 'Skeletal_and_or_Smooth_Muscle', 'Heart', 'Lower_Limb', 'Nerve', 'Spine_Sacral', 'Pancreas', 'Coronary_Artery', 'Arterial', 'Urinary_Bladder', 'Pulmonary_Artery', 'Biomedical_Device', 'Cerebrospinal_Fluid_Pathway', 'Peritoneal_Sac', 'Venous', 'Bile_Duct', 'Spine_Lumbar', 'Mesentery', 'Adrenal_Gland', 'Spine_Cervical', 'Spine_Cord', 'Mediastinal', 'Ovary', 'Pericardial_Sac', 'Large_Intestine', 'Brain', 'Mouth', 'Female_Genital_Structure', 'Retroperitoneal', 'Spine_Unspecified', 'Uterus', 'Testis', 'Neck', 'Lung', 'Prostate', 'Kidney', 'Undetermined', 'Pelvis', 'Bone_and_or_Joint', 'Upper_Limb', 'Stomach', 'Esophagus', 'Missing_Anatomy', 'Breast', 'Tracheobronchial', 'Thyroid', 'Liver']
        elif args.data_dir.find('indication_type')!=-1:
            self.ner_label_list =['NIL', 'nonneoplastic_dx', 'neoplastic_dx', 'symptom', 'trauma']
        elif args.data_dir.find('size_only')!=-1:
            self.ner_label_list =['NIL', 'current', 'past']
        elif args.data_dir.find('size_trend')!=-1:
            self.ner_label_list =['NIL', 'decreasing', 'new', 'no_change', 'disappear', 'increasing']
        else:
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    # results = {'f1':  0, 'f1_overlap': 0, 'precision': 0, 'recall': 0}
    ...
    if args.output_results:
        ...
        if do_test:
            output_w = open(os.path.join(args.output_dir, '{}_ent_pred_test.json'.format(args.test_file.replace('.json',''))), 'w')
        else:
